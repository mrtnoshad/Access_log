--SELECT count(DISTINCT pat_enc_csn_id_coded) from
SELECT * FROM
(select 
    ...
    where op.display_name like 'Patient on TPA%'
      ...
      and om.med_description LIKE 'ALTEPLASE%' -- ALTEPLASE 100mg infusion
    ...
    order by jc_uid, emergencyAdmitTime
    
   ) as cohort 
   where datetime_diff(cohort.emergencyAdmitTime, cohort.tpaAdminTime, MINUTE) >= -360
